...
title_html = f"""
    <h1 style="font-family:'Helvetica Neue'; color:#f2a900;">
        <img src="https://upload.wikimedia.org/wikipedia/commons/e/e8/Binance_Logo.svg" alt="Binance" width="50" height="50" style="vertical-align:bottom;margin-right:10px;"> 
        Binance Futures Swing Analysis
    </h1>
"""
# Render the title with the Binance icon
...
#@st.cache_data
# Function to identify swing high values before low
def get_short_swing_high(highs, sw_low_id, len_highs, lows, smooth_factor, current_price, swing_percent):
    # Figure out Swing Highs value, occurring before Lowest value
    ...
    for _peak in np.flip(_peaks):
        _peak_values = highs[max(_peak - smooth_factor + 1, 0):_peak + 1]

        # Figure out Highest value
        swing_high = np.max(_peak_values)
        sw_high_id = max(_peak - smooth_factor + 1, 0) + np.argmax(_peak_values)
        ...
        if swing_high >= (current_price * (100 + swing_percent) / 100):
            ...
            if current_price >= max(fib_levels):
                print("Current price is above the maximum Fibonacci level. Searching for preceding swing highs...")
                continue
            else:
...
#@st.cache_data
# Function to identify swing high values after low
def get_short_swing_high_after_low(highs, sw_low_id, len_highs, lows, smooth_factor, current_price, swing_percent):
    ...
    # Smoothen the values to remove jaggedy-ness
    _smooth_highs = _moving_average(highs[0:len_highs], smooth_factor)
    _potentials_highs = _smooth_highs[sw_low_id:]  # this line is modified
    # Find the local maximas in the smoothened curve
    _peaks, _ = find_peaks(_potentials_highs)
...
@st.cache_data
def plot_candlestick_chart_with_time_and_sma_and_swing_low_and_swing_high_long(symbol, time_period, timestamps, open_prices,
                                                                          high_prices, low_prices, close_prices, sma,
                                                                          smooth_factor, swing_percent):
    # Convert timestamps to datetime objects
    datetime_objects = [datetime.utcfromtimestamp(ts / 1000.0) for ts in timestamps]
...
st.sidebar.header(':blue[ ⚙️ Input parameters]', divider='rainbow')
...
def fetch_table_data(symbol, time_period):
    timestamps, open_prices, high_prices, low_prices, close_prices, _, _ = fetch_candle_data(symbol, time_period,candles=candles_to_plot)
    ...
    if timestamps is not None:
        ...
        # Identify swing highs
        sw_high_id, swing_high = get_short_swing_high(high_prices, swing_low_indices[-1], len(high_prices),
...
#@st.cache_data
# Function to fetch data for the table
def fetch_table_data_long(symbol, time_period):
    timestamps, open_prices, high_prices, low_prices, close_prices, _, _ = fetch_candle_data(symbol, time_period,candles=candles_to_plot)
...
title_text = "BEARISH 📉"
# Use HTML formatting to align the text in the center
title_html = f'<h1 style="text-align:center;">{title_text}</h1>'
# Render the title with centered alignment
st.markdown(title_html, unsafe_allow_html=True)
...
for index, row in enumerate(table_data):
    ...
    place_order_button = col9.button("Sell", key=f"place_order_{index}")
...
if place_order_clicked and selected_combination is not None:
    ...
    _, _, high_prices, _, close_prices, _, _ = fetch_candle_data(symbol, time_period,candles=candles_to_plot)
...
if view_plot_clicked and selected_combination is not None:
    ...
    timestamps, open_prices, high_prices, low_prices, close_prices, sma, _ = fetch_candle_data(selected_symbol, selected_time_period,candles=candles_to_plot)
...
# Populate table_data with data for each combination
for symbol, time_period in combinations:
    last_price, swing_high, swing_low, fib_level_0_382, fib_level_0_5, fib_level_0_618 = fetch_table_data_long(symbol, time_period)
    table_data.append([symbol, time_period, last_price, swing_high, swing_low, fib_level_0_382, fib_level_0_5, fib_level_0_618])
...
for index, row in enumerate(table_data):
    ...
    # Add "Place Order" and "View Plot" buttons for each row
    # Add "Place Order" and "View Plot" buttons for each row with unique keys
    place_order_button = col9.button("buy", key=f"place_order_{index}_{row[0]}_{row[1]}")
    view_plot_button = col10.button("📊", key=f"view_plot_{index}_{row[0]}_{row[1]}")
...
if place_order_clicked and selected_combination is not None:
    ...
    # Fetch additional data for order placement
    _, _, high_prices, low_prices, close_prices, _, _ = fetch_candle_data(symbol, time_period,candles=candles_to_plot)
...
if view_plot_clicked and selected_combination is not None:
    ...
    if timestamps is not None:
        # Display the plot under the table
        st.subheader(f'Plot for {selected_symbol} - {selected_time_period}')
        plot_candlestick_chart_with_time_and_sma_and_swing_low_and_swing_high_long(selected_symbol, selected_time_period, timestamps, open_prices,
                                                                              high_prices, low_prices, close_prices, sma,
                                                                              smooth_factor, swing_percent)
    else:
        st.warning(f"No data available for {selected_symbol} - {selected_time_period}")
